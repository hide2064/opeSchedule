...
for col_text, cx in [("色", 2.95), ("モデル名 / プロジェクト名", 3.2), ("状態", 5.6),
                      ("Ver", 6.7), ("最終更新", 7.35), ("操作", 8.8)]:
    add_text(sl, col_text, cx, 2.98, 1.6, 0.32, size=10, bold=True, color=C_PRIMARY)

# サンプル行（モデル名 / プロジェクト名 の横並び表示）
rows = [
    ("■", "ModelA", "ECサイトリニューアル", "作業中", "v3", "03/20", "✎ 🗑"),
    ("■", "", "クラウド移行プロジェクト", "未開始", "v1", "03/18", "✎ 🗑"),
    ("■", "ModelB", "セキュリティ監査", "終了", "—", "02/28", "✎ 🗑"),
]
for ri, (dot, model, name, status, ver, date, ops) in enumerate(rows):
    ry = 3.38 + ri * 0.5
    ...
    if model:
        add_text(sl, model + " /", 3.2, ry+0.1, 1.1, 0.3, size=9, bold=True, color=C_MUTED)
        add_text(sl, name, 4.0, ry+0.1, 1.5, 0.3, size=11, color=C_PRIMARY)
    else:
        add_text(sl, name, 3.2, ry+0.1, 2.3, 0.3, size=11, color=C_PRIMARY)
    add_text(sl, status, 5.6,  ry+0.1, 1.0, 0.3, size=10, color=C_DARK)
    add_text(sl, ver,    6.7,  ry+0.1, 0.6, 0.3, size=11, bold=True, color=C_ACCENT)
    add_text(sl, date,   7.35, ry+0.1, 1.2, 0.3, size=10, color=C_MUTED)
    add_text(sl, ops,    8.8,  ry+0.1, 0.7, 0.3, size=11, color=C_WARN)
...
info_card(sl, "プロジェクト設定項目", [
    ...
    "客先名         顧客企業名",
    "ベースPJ       参照元プロジェクト名",
    "サムネイル画像 PNG/JPEG（任意・ヘッダーに表示）",
    "表示モード     Day/Week/Month/Quarter",
], 6.5, 1.9, 6.5, 3.75)

# アーカイブルール
add_rect(sl, 6.5, 5.8, 6.5, 1.35, RGBColor(0xFF, 0xF3, 0xCD))
add_rect(sl, 6.5, 5.8, 6.5, 0.4, RGBColor(0xF3, 0x9C, 0x12))
add_text(sl, "⚡  アーカイブ自動化ルール",
         6.6, 5.83, 6.3, 0.35, size=13, bold=True, color=C_WHITE)
add_text(sl, "業務ステータスを「中断」または「終了」に変更すると\n"
             "自動的にアーカイブ状態（非表示）になります。\n"
             "Top 画面の「アーカイブ済みを表示 □」で再表示可能。",
         6.6, 6.25, 6.3, 0.82, size=12, color=C_DARK)
...
# サムネイル画像（小矩形で模示）
add_rect(sl, 0.9, 1.96, 0.55, 0.38, RGBColor(0x5A, 0xA0, 0xE9))
add_text(sl, "🖼", 0.9, 1.97, 0.55, 0.35, size=12, align=PP_ALIGN.CENTER, color=C_WHITE)
# モデル名 / プロジェクト名 表示
add_text(sl, "ModelA  /", 1.5, 2.0, 1.0, 0.38, size=9, color=RGBColor(0xCC, 0xDD, 0xFF))
add_text(sl, "ECサイトリニューアル", 2.5, 1.98, 2.2, 0.42, size=11, bold=True, color=C_WHITE)
btns_rest = [("[Day][Week][Month][Quarter]", 5.5), ("+ Add  JSON  CSV  📋履歴[2]", 9.8)]
for txt, bx in btns_rest:
    add_text(sl, txt, bx, 2.0, 2.8, 0.42, size=10, color=C_WHITE)
add_text(sl, "← Top", 0.35, 2.0, 0.6, 0.38, size=9, color=C_WHITE)
...
info_card(sl, "操作手順", [
    ...
    "③「まとめて表示」ボタンをクリック",
    "④ プロジェクト間にセパレーター行が挿入され",
    "   全 PJ のタスクがまとめて表示される",
], 0.4, 2.45, 6.0, 2.3, title_bg=C_MUTED)

# セパレーター行説明
add_rect(sl, 0.4, 4.85, 6.0, 1.15, RGBColor(0xEA, 0xF4, 0xFB))
add_text(sl, "📌  セパレーター行",
         0.5, 4.88, 5.8, 0.35, size=12, bold=True, color=C_PRIMARY)
add_text(sl, "プロジェクトとプロジェクトの間に\n"
             "プロジェクト名を示す区切り行が自動挿入されます。\n"
             "モデル名 / プロジェクト名 の形式で識別可能。",
         0.5, 5.25, 5.8, 0.7, size=11, color=C_DARK)

add_rect(sl, 0.4, 6.1, 6.0, 1.05, RGBColor(0xFF, 0xF3, 0xCD))
add_text(sl, "⚠  比較モードの制限",
         0.5, 6.13, 5.9, 0.35, size=12, bold=True, color=C_WARN)
add_text(sl, "・タスク追加・Export ボタン非表示\n"
             "・タスク詳細は読み取り専用（編集不可）\n"
             "・ドラッグ・クリティカルパス・依存矢印無効",
         0.5, 6.5, 5.9, 0.62, size=11, color=C_DARK)
